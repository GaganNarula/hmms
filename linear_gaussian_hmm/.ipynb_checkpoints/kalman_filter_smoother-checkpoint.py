...
def kalman_filter(x, params)
    ...
    for t in range(T):
        if t == 1:
            ...
            # FROM BISHOP BOOK (POSSIBLY WRONG)
            #K(t,:,:) = squeeze(P(t-1,:,:)) * C' / (C*squeeze(P(t-1,:,:))*C' + R);
            #mu(:,t) = A*mu(:,t-1) + squeeze(K(t,:,:))*(x(:,t) - C*A*mu(:,t-1));
            #V(t,:,:) = (eye(p,p) - squeeze(K(t,:,:))*C )*squeeze(P(t-1,:,:));
            #P(t,:,:) = A*squeeze(V(t,:,:))*A'+ Q;
